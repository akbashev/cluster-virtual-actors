...
struct HashRingTests {
  ...
  func testHashRing() {
    ...
    #expect(hashRing.getNode(for: "key1".hashValue) != nil, "Test 1 Failed: Key should be assigned to a node.")
    ...
    #expect(reassignedNodeAfterReaddition == initialAssignedNode, "Test 4 Failed: Key mapping not consistent after re-adding the node.")
    ...
    for key in keys {
      #expect(hashRing.getNode(for: key) == nil, "Test 6 Failed: Keys should not map to any node after all nodes are removed.")
    }
